...
Program takes input and places it in a buffer to index, finding unique words and returning their index in the buffer, length and count of each.
*/
...
void index_buffer(char* buffer, int indexarr[INDEX_SIZE][3], int *word_count, int buf_counter);
...
int main(void) {
    ...
    index_buffer(buffer, indexarr, &word_count, buf_counter);
